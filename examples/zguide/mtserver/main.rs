...
fn worker_routine(context : & zmq::Context){
    ...
    receiver.connect("inproc://workers").expect("failed to connect worker");
    loop {
        receiver.recv_string(0).expect("worker failed receiving").unwrap();
...
fn main(){
    ...
    clients.bind("tcp://*:5555").expect("failed to bind client router");
    workers.bind("inproc://workers").expect("failed to bind worker dealer");
