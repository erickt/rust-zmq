...
fn step1(context : & zmq::Context){
    ...
    xmitter.connect("inproc://step2").expect("step 1 failed connecting");
...
fn step2(context : & zmq::Context){
    ...
    receiver.bind("inproc://step2").expect("failed binding step 2");
    ...
    let xmitter = context.socket(zmq::PAIR).unwrap();
    xmitter.connect("inproc://step3").expect("step 2 failed connecting");
...
fn main(){
    ...
    let receiver = context.socket(zmq::PAIR).unwrap();
    receiver.bind("inproc://step3").expect("failed binding step 3");
    let ctx = context.clone();
